...
def train_cnn(X_train, y_train, epochs=100, batch_size=32):
    """Train CNN model."""
...
def model_section():
    ...
    with col1:
        ...
        model_type = st.selectbox("Select Model Type", ["Random Forest", "CNN"])
        ...
        if model_type == "Random Forest":
            ...
            epochs = st.slider("Number of epochs", 50, 200, 100, 10)
        ...
        with st.spinner("Training model..."):
            try:
                ...
                if model_type == "Random Forest":
                    ...
                    # Train CNN model
                    model, history = train_cnn(X_train_scaled, y_train, epochs, batch_size)
                    ...
                    fig = px.line(history_df, title='Model Training History')
                    fig.add_scatter(y=history_df['loss'], name='Training Loss')
                    if 'mae' in history_df.columns:
                        fig.add_scatter(y=history_df['mae'], name='MAE')
                    st.plotly_chart(fig, use_container_width=True)
                
                # Evaluate model
                if model_type == "CNN":
                    # For CNN, we need to reshape the data for evaluation
                ...
                # Add cross-validation results for Random Forest only
                if model_type == "Random Forest":
                    add_cross_validation(model, X_train_scaled, y_train)
